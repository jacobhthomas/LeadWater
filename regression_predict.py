...
for col in ['X1st.Draw', 'X2nd.Draw', 'X3rd.Draw',
    ...
    df.drop(df[df[col] > 50].index, inplace=True)

y = df['avg'].values.reshape(-1, 1)

# print(df.sample(4))

df_pred = df[['X1st.Draw', 'X2nd.Draw', 'X3rd.Draw',
       'X4th.Draw', 'X5th.Draw', 'X6th.Draw', 'X7th.Draw', 'X8th.Draw',
       'X9th.Draw', 'X10th.Draw', 'X11th.Draw']]
...
model.fit(df_pred,df_response)

print('r_sq: ', model.score(df_pred, df_response))
